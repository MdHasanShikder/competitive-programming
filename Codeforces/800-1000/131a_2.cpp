...
int main(){
    ...
    int count1=0;  //handling the two situations if all characters all uppercase or if the first character is lower and rest all is uppercase

    if(len>1){ //for the second case (first letter is lower then rest all are uppercase)
        for(int i=1; i<len; i++){
            if(isupper(word[i])){
                count1++;
            }
    ...
    if(len==1 || (count1==len-1)){ //combining the three cases
        for(int i=0; i<len; i++){
